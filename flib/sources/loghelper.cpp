...
#if defined(_MSC_VER) || defined(_WIN32) || defined(_WIN64) || defined(WIN32) || defined(WIN64)
#define _WIN_
#endif
...
void log_helper::timer_start(std::string key)
{
	timer_start_points[key] = std::chrono::system_clock::now();
}
void log_helper::timer_output(std::string key)
{
	if (timer_start_points.find(key) == timer_start_points.end())
	{
		LOG_DEBUG("timer(%s) : null, not start", key);
		return;
	}
	auto end = std::chrono::system_clock::now();
	LOG_DEBUG("timer(%s) : process time: %dms", key, std::chrono::duration_cast<std::chrono::milliseconds>(end - timer_start_points[key]).count());
	timer_start_points.erase(key);
}